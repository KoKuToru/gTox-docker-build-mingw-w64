...
RUN ls -lisa /tmp
RUN su -c "cd /tmp/gTox-docker-build-mingw-w64/mingw-w64-freetype/ && makepkg -si --noconfirm" -s /bin/bash nobody

#install pango
RUN su -c "yaourt -Sy --noconfirm --m-arg \"--skipchecksums --skippgpcheck\" mingw-w64-harfbuzz mingw-w64-cairo" -s /bin/bash nobody
RUN su -c "cd /tmp/gTox-docker-build-mingw-w64/mingw-w64-pango/ && makepkg -si --noconfirm" -s /bin/bash nobody
...
#install glibmm
RUN su -c "cd /tmp/gTox-docker-build-mingw-w64/mingw-w64-glibmm/ && makepkg -si --noconfirm --skipchecksums --skippgpcheck" -s /bin/bash nobody

#install atkmm
RUN su -c "cd /tmp/gTox-docker-build-mingw-w64/mingw-w64-atkmm/ && makepkg -si --noconfirm --skipchecksums --skippgpcheck" -s /bin/bash nobody

#install cairomm
RUN su -c "cd /tmp/gTox-docker-build-mingw-w64/mingw-w64-cairomm/ && makepkg -si --noconfirm --skipchecksums --skippgpcheck" -s /bin/bash nobody
